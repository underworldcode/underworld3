...
#align(center + horizon, cetz.canvas(length: 1.5cm, {
  ...
  let triad-stroke = 0.9pt + black
  let triad-mark = (end: ">", fill: black)
  let triad-arrow(dir-2d, label-offset-factor, lbl, lbl-anchor, label-dy: 0) = {
    let tip = (
    ...
    let lbl-pos = (
      ...
      triad-origin.at(1) + label-offset-factor * TRIAD-LEN * dir-2d.at(1) + label-dy,
    )
    content(lbl-pos,
            text(fill: black, size: 10pt, style: "italic", lbl),
            anchor: lbl-anchor)
  }
  triad-arrow(AXIS-X-2D, 1.10, $x$, "north-west")
  triad-arrow(AXIS-Y-2D, 1.30, $y$, "north-east", label-dy: -0.045)
  triad-arrow(AXIS-Z-2D, 1.22, $z$, "south",      label-dy: -0.045)

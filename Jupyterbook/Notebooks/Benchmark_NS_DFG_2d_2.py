...
with pipemesh.access(v_stokes, v_soln):
    ...
    v_soln.data[...] *= 1.0 + 0.1 * np.cos(v_soln.coords[:,1].reshape(-1,1) / 0.41 * np.pi) 
...
#     point_cloud = pv.PolyData(points)
    ...
    pl.add_mesh(pvmesh, cmap="coolwarm", edge_color="Black", show_edges=True, scalars="Vmag",
...
for step in range(0,500):
    ...
    with passive_swarm.access(passive_swarm.particle_coordinates):
        for i in range(npoints):
            passive_swarm.particle_coordinates.data[-1:-(npoints+1):-1,:] = np.array([0.0, 0.19] + 0.02 * np.random.random((npoints,2)))

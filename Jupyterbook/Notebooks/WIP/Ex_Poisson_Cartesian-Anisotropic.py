...
with mesh.access():
    ...
    if not np.allclose(poisson.u.data[:, 0], exact[:], rtol=7.0e-2):
...
mesh = uw.discretisation.Box(elementRes=(9, 9), minCoords=(-2.2, -0.4))
bnds = mesh.boundary
# Create Poisson object
u_degree = 1
...
y1 = mesh.maxCoords[1]
y0 = mesh.minCoords[1]
diff = uw.discretisation.MeshVariable(
    mesh=mesh, num_components=1, name="diff", vtype=uw.VarType.SCALAR, degree=u_degree
)

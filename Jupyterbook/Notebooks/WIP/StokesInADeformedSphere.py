...
r_i = 0.0
r_o = 1.0
meshball = uw.meshes.SphericalShell(dim=3, radius_inner=r_i, radius_outer=r_o, cell_size=cell_size, verbose=False)
# -
...
if uw.mpi.size == 1:
    ...
    # pl.add_mesh(pvmesh,'Black', 'wireframe', opacity=0.5)
    pl.add_mesh(clipped, cmap="coolwarm", edge_color="Black", show_edges=True, use_transparency=False, opacity=1.0)
...
poisson_vr = uw.systems.Poisson(meshball, u_Field=vr_soln, solver_name="poisson_vr", degree=2, verbose=False)
poisson_vr.k = k
poisson_vr.f = h
...
if uw.mpi.size == 1:
    ...
    pl.add_mesh(
        clipped, cmap="coolwarm", edge_color="Black", show_edges=True, scalars="Vr", use_transparency=False, opacity=1.0
    )

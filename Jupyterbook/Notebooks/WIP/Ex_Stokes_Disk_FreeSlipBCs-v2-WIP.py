...
meshball = uw.meshes.SphericalShell(
    dim=2, degree=1, radius_inner=0.0, radius_outer=1.0, cell_size=res, cell_size_upper=0.75 * res
)
...
# -
with meshball.access(mask):
    mask.data[:, 0] = uw.function.evaluate(surface_fn, mask.coords)


# +

buoyancy_force = Rayleigh * gravity_fn * t_init
buoyancy_force -= Rayleigh * v_soln.fn.dot(unit_rvec) * surface_fn

stokes.PF0 = v_soln.fn.dot(unit_rvec) * mask.fn
stokes.bodyforce = unit_rvec * buoyancy_force
stokes.penalty = 0.0

# This may help the solvers - penalty in the preconditioner
stokes._Ppre_fn = 1.0 / (stokes.viscosity + 100.0 * surface_fn)
...
if uw.mpi.size == 1:
    ...
    pl.add_mesh(
        pvmesh, cmap="coolwarm", edge_color="Black", show_edges=True, scalars="P", use_transparency=False, opacity=0.5
    )

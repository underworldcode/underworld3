...
cubed_sphere_mesh_shell = uw.discretisation.StructuredCubeSphereShellMesh(
    elementRes=(11, 5), radius_inner=0.5, radius_outer=1.0, simplex=False
)


cubed_sphere_mesh_ball = uw.discretisation.StructuredCubeSphereBallMesh(elementRes=9, radius_outer=1.0, simplex=True)
...
stokes.add_dirichlet_bc((0.0, 0.0, 0.0), cubed_sphere_mesh.boundary.ALL_BOUNDARIES, (0, 1, 2))
...
# +
pl = pv.Plotter()
pl.add_mesh(clipped, cmap="coolwarm", edge_color="Black", show_edges=True, scalars="density", use_transparency=False)

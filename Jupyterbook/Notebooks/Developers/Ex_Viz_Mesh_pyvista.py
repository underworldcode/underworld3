...
qmesh2 = uw.discretisation.Hex_Box(
    dim=3, elementRes=elementRes, minCoords=minCoords, maxCoords=maxCoords, cell_size=10.0
)
...
s_s_mesh = uw.discretisation.Simplex_Box(
    dim=3, elementRes=elementRes, minCoords=minCoords, maxCoords=maxCoords, cell_size=0.05
)
...
u_s_mesh = uw.discretisation.Unstructured_Simplex_Box(
    dim=3,
    minCoords=minCoords,
    maxCoords=maxCoords,
...
u_s_2d_mesh = uw.discretisation.Unstructured_Simplex_Box(
    dim=2,
    minCoords=minCoords,
    maxCoords=maxCoords,
...
s_s_2d_mesh = uw.discretisation.Simplex_Box(
    dim=2,
    minCoords=minCoords,
    maxCoords=maxCoords,
...
s_q_2d_mesh = uw.discretisation.Hex_Box(
    dim=2,
    minCoords=minCoords,
    maxCoords=maxCoords,
    elementRes=elementRes,
...
density = sympy.Piecewise((dens_ball, abs_r < radius_ball**2), (dens_other, True))
density
...
# %%
cubed_sphere_meshio = uw.discretisation.StructuredCubeSphereBallMesh.build_pygmsh(
    elementRes=33, radius_outer=1.0, simplex=True
)
...
pl.add_mesh(clipped_stack, cmap="coolwarm", edge_color="Black", show_edges=True, use_transparency=False)
pl.show()


# %%
cubed_sphere_shell_meshio = uw.discretisation.StructuredCubeSphereShellMesh.build_pygmsh(
    elementRes=(33, 16), radius_inner=0.5, radius_outer=1.0
)
...
qmesh2 = uw.discretisation.Hex_Box(
    dim=3, elementRes=elementRes, minCoords=minCoords, maxCoords=maxCoords, cell_size=0.5
)

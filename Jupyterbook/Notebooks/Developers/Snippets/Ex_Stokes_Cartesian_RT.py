...
def do_uw3():
    ...
    stokes.viscosity = Piecewise(
        (viscosityRatio, Abs(matSwarmVar.fn - lightIndex) < 0.5),
    ...
    while time < model_end_time:
        ...
        if MPI.COMM_WORLD.rank == 0:
            print(
                f"Step {str(step).rjust(3)}, time {time:6.2f}, vrms {vrms_val:.3e}, Time(s): Solve {stime:5.2f}, Plot {ptime:5.2f}, Evaluate {etime:5.2f}, Advect {atime:5.2f}"
            )
...
if MPI.COMM_WORLD.rank == 0:
    ...
    if not np.allclose(uw3_maxvrms, expected_maxvrms, rtol=rtol):
        raise RuntimeError(
            f"Encountered max VRMS ({uw3_maxvrms}) not sufficiently close to expected value ({expected_maxvrms})"
        )
    if not np.allclose(uw3_maxvrms_time, expected_maxvrms_time, rtol=rtol):
        raise RuntimeError(
            f"Encountered max VRMS time ({uw3_maxvrms_time}) not sufficiently close to expected value ({expected_maxvrms_time})"
        )

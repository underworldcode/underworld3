...
# # Read checkpoint
#
# Read in some checkpoint data on 1 proc:
#
...
#   
# The grid file has re-ordered vertices and this puts it out of sync with the original dm an the data as dumped by the vector viewer.
#
# However, if we use the orig.msh.h5 to provide the ordering, then it works on 1 process. 
#
...
# %%
np_read = 2
expt_name = f"test_checkpointing_np{np_read}"
...
indexc = uw.discretisation.MeshVariable("Indexc", mesh, 1, degree=1, continuous=True)
...
with mesh.access(p,v, vc, pc, index, indexc):
    p.data[:,0] = p.coords[:,0]
    v.data[:,:] = v.coords[:,:]
    vc.data[:,:] = 0.0
    pc.data[:,0] = 0.0    
    index.data[:,0] = indices.astype(float)    
    indexc.data[:,0] = 0.0

# %%
with mesh.access(vc, pc, indexc):
    vc.data[:,:] = 0.0
    pc.data[:,0] = 0.0    
    indexc.data[:,0] = 0.0
...
sec = mesh.dm.getSection()
lvec = mesh.dm.getLocalVec()
gvec = mesh.dm.getGlobalVec()

# push this vec into the big one 

isset, subdm = mesh.dm.createSubDM(indexc.field_id)
pcvec = gvec.getSubVector(isset)
subdm.localToGlobal(index._lvec, pcvec)
...
pcvec = gvec.getSubVector(isset)
indexc._gvec.array[...] = pcvec.array[...]
subdm.globalToLocal(index._gvec, index._lvec)

newsec.view()

# if uw.mpi.rank == 1:
#     print(f"Index Permutation - {index._lvec.array[0:20].astype(int).T}", flush=True)
...
with mesh.access(p,v, vc, pc):
    if uw.mpi.rank == 0:
        ...
        print(f"Ixc - {indexc.data[0:10].astype(int).T}", flush=True)
        print(f"Ix  - {index.data[0:10].astype(int).T}", flush=True)

...
minCoord = (-1, -1)
maxCoord = (1, 1)
mesh = uw.meshes.Box(elementRes=(n_els, n_els), minCoords=minCoord, maxCoords=maxCoord)
# %%
v_degree = 1
...
matvar = uw.discretisation.MeshVariable(
    mesh=mesh, num_components=1, name="mat", vtype=uw.VarType.SCALAR, degree=v_degree
)

inside = mesh.data[:, 0] ** 2 + mesh.data[:, 1] ** 2 < 0.5**2
with mesh.access(matvar):
    matvar.data[:, 0] = np.where(inside, 1.0, 0.0)

# %%
# free slip.
# note with petsc we always need to provide a vector of correct cardinality.
bnds = mesh.boundary
stokes.add_dirichlet_bc((0.0, 0.0), [bnds.TOP, bnds.BOTTOM], 1)  # top/bottom: components, function, markers
stokes.add_dirichlet_bc((0.0, 0.0), [bnds.LEFT, bnds.RIGHT], 0)  # left/right: components, function, markers
...
try:
    ...
    exit(1)
mesh2 = uw2.mesh.FeMesh_Cartesian(elementRes=(n_els, n_els), minCoord=minCoord, maxCoord=maxCoord)
...
fField.data[:, 1] = np.where(inside, 1.0, 0.0)

# %%
coord = fn.input()
fn_sphere = fn.math.dot(coord, coord) < 0.5**2
conditions = [(fn_sphere, (0.0, 1.0)), (True, (0.0, 0.0))]
...
freeslipBC = uw2.conditions.DirichletCondition(variable=v2Field, indexSetsPerDof=(iWalls, jWalls))

stokes2 = uw2.systems.Stokes(
    velocityField=v2Field, pressureField=pField, conditions=freeslipBC, fn_viscosity=1.0, fn_bodyforce=fn_buoyancy
)

...
with meshball.access(r):
    r.data[:,0]   = uw.function.evaluate(sympy.sqrt(x**2+y**2+z**2), meshball.data, meshball.N)  # cf radius_fn which is 0->1 
...
with meshball.access(t_soln):
    t_soln.data[...] = (uw.function.evaluate(sympy.sin(radius_fn*np.pi)*radius_fn**2, t_soln.coords, meshball.N) * qt_vals).reshape(-1,1)
...
meshball.write_timestep_xdmf(savefile, 
                             meshUpdates=True,

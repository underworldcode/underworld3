...
if uw.mpi.rank==0:
    ...
    with pygmsh.geo.Geometry() as geom:
        ...
        domain = geom.add_rectangle(xmin=-width,ymin=-height, xmax=width, ymax=height, z=0, holes=[inclusion], mesh_size=csize)
...
mesh1 = uw.mesh.Mesh("tmp_ps_shear_inclusion.msh", simplex=True)
mesh1.dm.view()
...
t_soln           = uw.mesh.MeshVariable('T',      mesh1, 1,  degree=2 )
...
stokes = uw.systems.Stokes(mesh1, 
...
stokes.viscosity = sympy.Min(tau_y / stokes._Einv2, 1.0)

stokes.solve(zero_init_guess=False)
# -


nodal_visc_calc.uw_function
...
if uw.mpi.size==1:
    ...
    pl.add_mesh(pvmesh, cmap="coolwarm", edge_color="Black", show_edges=True, scalars="Edot",
                  use_transparency=False, opacity=1.0) #clim=[0.0,1.0])

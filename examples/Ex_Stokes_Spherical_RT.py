...
def plot_mesh(mesh,colour_data=None):
    ...
    kplot += k3d.mesh(vertices,indices,color_map=k3d.colormaps.basic_color_maps.RainbowDesaturated,wireframe=True, **options)
...
def plot_swarm(swarm,colour_data=None,point_size=0.02,point_shader='dot'):
    ...
    with swarm.access():
        ...
        pts = k3d.points(positions=coords,point_size=point_size,color_map=k3d.colormaps.basic_color_maps.RainbowDesaturated, **options)
...
with swarm.access(density):
    ...
    for index,coord in enumerate(swarm.data):
        ...
        if r > (0.75 + 0.02*np.sin(8.*theta)):
            density.data[index] = 10.

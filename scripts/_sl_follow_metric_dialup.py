"""Sweep refinement higher (2, 3, 5, 7) on the stagnant-lid step
to show that increasing the refinement dial does focus more on
the gradients."""
import os
import numpy as np
...
OUT = os.path.expanduser(
    '~/+Simulations/StagnantLid/follow_metric_dialup')
os.makedirs(OUT, exist_ok=True)

REFS = [2.0, 3.0, 5.0, 7.0]
...
for ref in REFS:
    ...
    if os.path.exists(
        ...
        print(f"{tag}: already adapted")
        continue
    print(f"{tag}: adapting")
    m, T = load_fresh()
...
# Render — 4-panel grid at high res
ncols = 2
nrows = 2
...
for i, ref in enumerate(REFS):
    ...
    pl.subplot(row, col)
    pl.add_text(f"refinement = {ref}", font_size=28, color='black')
...
out_png = os.path.join(OUT, "plot_dialup.png")

"""Sweep raw-mode metric combinations on the step-80 R1/uniform
T field, then render T (no mesh) and |∇T| + mesh side by side.

Each combination is a strategic choice:
  (power, R) → adaptation behaviour
  p=1, R=1.5: front-following baseline, modest grading
  p=1, R=3:   front-following, more grading allowed
  p=2, R=3:   gradient-uniform target, moderate clamp
  p=2, R=5:   gradient-uniform, larger clamp (probably the
              best raw-mode operating point)
  p=2, R=7:   gradient-uniform, very large clamp (boundary
              cap territory)
"""
import os
import numpy as np
...
OUT_BASE = os.path.expanduser(
    '~/+Simulations/StagnantLid/raw_metric_sweep')
...
# Adapt for each combo and FE-remap T so visualised T is identical
for label, p, R in COMBOS:
...
for label, p, R in COMBOS:
    ...
    m = uw.discretisation.Mesh(
        os.path.join(out, "adapted.mesh.00000.h5"))
    T = uw.discretisation.MeshVariable(
        f"T_view_{label}", m, vtype=uw.VarType.SCALAR,
        degree=3, continuous=True)
    T.read_timestep("adapted", "T_v2p1", 0, outputPath=out)
    loaded.append((label, p, R, m, T))
...
for c, (label, p, R, m, T) in enumerate(loaded):
    title = f"raw  p={p}  R={R}"
    # Row 0: T field
    pv_T = vis.meshVariable_to_pv_mesh_object(T)
...
out = os.path.join(OUT_BASE, "plot_raw_sweep.png")
pl.screenshot(out)
pl.close()
print(f"wrote {out}")

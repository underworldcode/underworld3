"""Repeatability probe: same Stokes solve at R=1.5 run N times,
fresh build each time. Quantify the noise floor in wall + iters."""
import os
import time
import numpy as np
import sympy
...
def one_solve():
    ...
    T = uw.discretisation.MeshVariable(
        f"T_v2p1_{id(m)}", m, vtype=uw.VarType.SCALAR,
        degree=3, continuous=True)
    V = uw.discretisation.MeshVariable(
        f"V_v2p1_{id(m)}", m, vtype=uw.VarType.VECTOR,
        degree=2, continuous=True)
    P = uw.discretisation.MeshVariable(
        f"P_v2p1_{id(m)}", m, vtype=uw.VarType.SCALAR,
...
print(f"{'#':>3} {'reason':>6} {'its':>4} {'wall':>8}  {'|v|max':>10}")
print("-" * 40)
walls, iters, vmaxs = [], [], []
for k in range(N_REPEATS):
    reason, its, wall, vmax = one_solve()
    print(f"{k:>3} {reason:>6} {its:>4} {wall:>7.2f}s  "
          f"{vmax:>10.3e}", flush=True)
    walls.append(wall); iters.append(its); vmaxs.append(vmax)

print("\nstats:")
print(f"  wall: median={np.median(walls):.2f}s  "
      f"range=[{min(walls):.2f}, {max(walls):.2f}]")
print(f"  its:  unique={set(iters)}")
print(f"  |v|max: range=[{min(vmaxs):.3e}, {max(vmaxs):.3e}]")

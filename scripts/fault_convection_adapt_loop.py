...
p.add_argument('--fault-refine-width', type=float, default=-1.0,
               help='Gaussian half-width of the fault refinement band. '
                    '< 0 (default) ⇒ 1.5·fault_width.')
...
tag = args.out_tag or (
    ...
    f"_{args.strategy}")
OUT_DIR = os.path.expanduser(f'~/+Simulations/StagnantLid/{tag}')
...
if resume_info is not None:
    ...
    if args.cell_outer > 0 and args.cell_inner > 0:
        ...
        mesh = uw.meshing.Annulus(
            ...
            qdegree=3, refinement=_ref)
    else:
        # uniform mesh (default) — full node budget for the mover to
        # redistribute toward the fault.
        mesh = uw.meshing.Annulus(
            radiusOuter=1.0, radiusInner=0.5,
            cellSize=1.0 / args.res, qdegree=3, refinement=_ref)
else:
    resume_step = 0
    resume_label = None
    mesh = uw.discretisation.Mesh(
...
def _refresh_fault_influence():
    ...
    *signed* distance as a P1 FE field; the sign FLIPS across the fault, so
    ...
    # Keep the Surface's own (signed) distance field fresh for the adapt metric.
...
# Faultless control: force isotropic FK rheology (finf=0 → eta_weak=eta_FK).
_rheology = "isotropic" if NO_FAULT else args.rheology
eta_FK = sympy.exp(theta_FK * (1 - T.sym[0]))
finf = 0 if NO_FAULT else gfac.sym[0]
...
def _adapt_step():
    ...
    if NO_FAULT:
        ...
        # Use the SIGNED distance — the gaussian squares it (so values are
        # identical to using |d|), but without Abs the mover's gradient-
        # projection JIT differentiates cleanly (Abs(d) emits `re` in C99).
        d_signed = fault.distance.sym[0]
        _gauss = sympy.exp(-(d_signed / refine_w) ** 2)
        # SCALAR fault density (ot-reset / non-mmpde / aniso-disabled + the
        # diagnostic mismatch). Isotropic: refines a corridor, centre coarse.
        fault_rho = 1.0 + args.fault_refine_amp * _gauss
        ...
        #   M = rho_T·I + (Rf²-1)·exp(-(d/W)²)·n nᵀ
        # the n nᵀ term refines ONLY the fault-normal direction, so cells go
        # thin-across / long-along and a node ROW lands on the centre-line.
        _Rf = float(args.fault_anisotropy)
        rho_tensor = (rho_T * sympy.eye(2)
                      + (_Rf ** 2 - 1.0) * _gauss * _nnT) if _Rf > 0 else None
    ...
    if args.method in ("mmpde", "variational"):
        mk = dict(step_frac=0.2, accel="cg", momentum=0.0)

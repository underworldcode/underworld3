"""Winslow + diffuser (top row) vs diffuser-only (bottom row),
both with CN + clamp + rk4. Same 40-step run.
"""
...
D_W = "/tmp/mdump_cn_long"        # CN + Winslow
D_NW = "/tmp/mdump_cn_nowinslow"  # CN, no Winslow
OUT = "/tmp/winslow_vs_diffuser_only.png"
...
ref_v = np.load(os.path.join(D_W, "step_0001.npz"))["coords"]
...
for col, s in enumerate(picks):
    for row, (label, d) in enumerate(
        ...
        # h_max (|h| max anywhere on rim)
        h_max = np.abs(bnd_r - 1.0).max()
        ttl = f"step {s}"
        if col == 0:
            ttl = f"[{label}]  " + ttl
        ax.set_title(
            f"{ttl}\nh_pole={h_pole:+.3f}  |h|max={h_max:.3f}",
            fontsize=8)

cbar = fig.colorbar(cf, ax=axes.ravel().tolist(),
                    shrink=0.75, pad=0.02, label="T")
fig.suptitle(
    "Winslow + diffuser (top) vs diffuser only (bottom) — "
    "rk4 + CN + monotone=clamp, res=16 Annulus, 40 steps  |  "
    "top sinks monotonically, bottom oscillates with worse mass drift",
    fontsize=11)

"""One-shot OT vs original mesh comparison on a late-time
snapshot from the stagnant-lid convection run.

Loads the mesh + T from a snapshot, builds the gradient-based
metric (refinement=3.0), runs smooth_mesh_interior(method='ot',
n_outer in {5,10}, relax=0.1, step_frac=0.3) on independent
copies, renders a 3-panel mesh-on-T comparison **incrementally**
(saves PNG after each variant so progress is visible), and
re-solves Stokes on each variant to report (vrms, Nu_surface).

Bit-identical-T fairness is achieved by reloading from disk for
each variant so the upstream mesh/T pair is fresh.
"""
...
p.add_argument("--step", type=int, default=25,
               help="Snapshot step to use (default 25 = the "
                    "cleanest late-time snapshot before the "
                    "step-30 T overshoot abort).")
...
p.add_argument("--refinement", type=float, default=3.0,
               help="Refinement knob for the gradient metric.")
p.add_argument("--out-dir", type=str,
               default="/tmp/ot_test_logs")
args = p.parse_args()
...
PNG_PATH = os.path.join(
    args.out_dir,
    f"ot_compare_{SNAPSHOT_LABEL}_r{int(args.refinement)}.png")

print(f"=== OT snapshot comparison: {SNAPSHOT_LABEL} from {SRC} ===",
      flush=True)
...
def load_state():
    """Fresh mesh+T+V+P from the snapshot files."""
...
def fe_remap_T(mesh, T, old_X, old_T_data, new_X):
    """Standard FE remap from old_X to new_X."""
...
def mesh_edges_xy(mesh):
    """Return (n_edges, 2, 2) Cartesian edge segments."""
    dm = mesh.dm
...
# 3-panel plot built incrementally. Each panel filled as its
# variant finishes; figure saved every update.
...
def render_panel(ax, mesh, T_field, title):
    ...
    lc = LineCollection(segs, colors="white", linewidths=0.25,
                        alpha=0.6)
...
def flush_fig():
    fig.suptitle(
        f"OT mesh-improvement on {SNAPSHOT_LABEL}  "
...
# --- (a) original ---
print("\n--- (a) original snapshot mesh ---", flush=True)
mesh_a, T_a, V_a, P_a = load_state()
print(f"  n_verts={mesh_a.X.coords.shape[0]}", flush=True)
...
rho_b = uw.meshing.metric_density_from_gradient(
    mesh_b, T_b, refinement=args.refinement, name="ot_compare_b")
t0 = time.time()
...
rho_c = uw.meshing.metric_density_from_gradient(
    mesh_c, T_c, refinement=args.refinement, name="ot_compare_c")
t0 = time.time()
uw.meshing.smooth_mesh_interior(
    mesh_c, metric=rho_c, method="ot",
    boundary_slip="box",
    method_kwargs=dict(n_outer=10, relax=0.1, step_frac=0.3),
    verbose=True)
print(f"  OT×10 wall {time.time() - t0:.1f}s", flush=True)
new_X_c = np.asarray(mesh_c.X.coords).copy()
fe_remap_T(mesh_c, T_c, old_X_c, old_T_c, new_X_c)
vrms_c, nu_c = stokes_solve(mesh_c, T_c, V_c, P_c, "OT×10")
render_panel(axes[2], mesh_c, T_c,
             f"(c) OT × 10\nvrms={vrms_c:.3e}  Nu={nu_c:.3f}")
flush_fig()

print("\n=== SUMMARY ===", flush=True)
print(f"  original  vrms={vrms_a:.3e}  Nu={nu_a:.3f}", flush=True)
print(f"  OT×5      vrms={vrms_b:.3e}  Nu={nu_b:.3f}", flush=True)
print(f"  OT×10     vrms={vrms_c:.3e}  Nu={nu_c:.3f}", flush=True)

np.savez(
    os.path.join(args.out_dir,
                 f"ot_compare_{SNAPSHOT_LABEL}"
                 f"_r{int(args.refinement)}.npz"),
    original=np.array([vrms_a, nu_a]),
    ot5=np.array([vrms_b, nu_b]),
    ot10=np.array([vrms_c, nu_c]),

...
def _adapt_step():
    ...
    Returns True iff the mesh was changed."""
    old_X = np.asarray(mesh.X.coords).copy()
    old_T = np.asarray(T.data).copy()
    # Compute background h0 from the current mesh (the
    # gradient-side smoothing length is set relative to it,
    # so it tracks any global resolution change).

"""Compare 1 vs 2 iterations: what does step 2 do to a clean
step 1? Try with frozen and refresh M tensor."""
import os
import sys
import numpy as np
...
OUT = os.path.expanduser(
    '~/+Simulations/StagnantLid/synthetic_shapes_iter2')
...
rho_expr = uw.meshing.metric_density_from_gradient(
    mesh_ref, T_ref, refinement=3.0, name="iter2_rho_view")
...
CASES = [
    ("1 iter, relax=1.0 (BASELINE)",
     dict(n_outer=1, relax=1.0)),
    ("2 iter, relax=1.0, FROZEN M",
     dict(n_outer=2, relax=1.0,
          metric_refresh_per_iter=False)),
    ("2 iter, relax=1.0, REFRESH M",
     dict(n_outer=2, relax=1.0,
          metric_refresh_per_iter=True)),
    ("2 iter, relax=0.5, FROZEN M",
     dict(n_outer=2, relax=0.5,
          metric_refresh_per_iter=False)),
    ("2 iter, relax=0.5, REFRESH M",
     dict(n_outer=2, relax=0.5,
          metric_refresh_per_iter=True)),
    ("2 iter, relax=0.2, FROZEN M",
     dict(n_outer=2, relax=0.2,
          metric_refresh_per_iter=False)),
]
...
def adapt_one(label, kw):
    ...
    rho = uw.meshing.metric_density_from_gradient(
        m, T, refinement=3.0, name=f"iter2_{label[:5]}")
...
out_png = os.path.join(OUT, "plot_shapes_iter2.png")

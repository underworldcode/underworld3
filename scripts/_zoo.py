...
def _step(state, scheme, dt_factor, dt_cap=None,
    ...
    def deform_by_inc(disp_internal):
        # Two interior-propagation strategies for the prescribed
        # surface increment:
        #
        #  'diffuser' (default, original): Fourier→sympy → solve a
        #     Poisson problem (inc_fn on Upper, 0 on Lower) and
        #     deform every node by the harmonic Vr·r̂ extension.
        #
        #  'winslow': move only the Upper-surface vertices radially
        #     by the increment, then relax the interior with the
        #     parallel-safe graph-Laplacian smoother (Upper pinned
        #     at its NEW position, Lower pinned). No Poisson solve —
        #     isolates the diffuser chain, which the bisection
        #     pinned as the sole remaining corruption suspect.
        mode = globals().get('_DEFORM_MODE', 'diffuser')
        if mode == 'surface-only':
            # CONTROL: move ONLY the Upper-surface vertices
            # radially by the increment; interior nodes frozen.
            # No diffuser, no Winslow, no interior propagation.
            # Mesh quality near the surface degrades badly, but
            # there is ZERO interior-node motion — so no
            # Lagrangian-carry-without-ALE error in the interior.
            coords = np.asarray(mesh.X.coords).copy()
            surf = coords[internal_idx]
            rr = np.sqrt(surf[:, 0] ** 2 + surf[:, 1] ** 2)
            rhat = surf / rr[:, None]
            di = np.asarray(disp_internal).reshape(-1)
            coords[internal_idx] = surf + di[:, None] * rhat
            mesh._deform_mesh(coords)
            return
        if mode == 'winslow':
            ...
            di = np.asarray(disp_internal).reshape(-1)
            coords[internal_idx] = surf + di[:, None] * rhat
            # Surface now moved, interior unchanged (distorted);
            # one rebuild, then Winslow relaxes the interior.
            mesh._deform_mesh(coords)
            smooth_mesh_interior(
                mesh, pinned_labels=["Upper", "Lower"],
                n_iters=globals().get('_WINSLOW_DEFORM_ITERS', 20),
                alpha=0.5)
            return
...
def main():
    ...
    p.add_argument('--smoother-impl', type=str, default='uw',
                   choices=['uw', 'in-script'],
                   help="Which smoother to use when winslow triggers: "
                   "'uw' = uw.meshing.smooth_mesh_interior (parallel-"
                   "safe PETSc Mat); 'in-script' = the original local "
                   "scipy graph-Laplacian (correct in serial only).")
    p.add_argument('--dump-mesh-dir', type=str, default=None,
                   help="If set, dump mesh.X.coords + T to "
                   "<dir>/step_NNNN.npz after each step. Used for "
                   "smoother-comparison plots.")
    p.add_argument('--deform-mode', type=str, default='diffuser',
                   choices=['diffuser', 'winslow', 'surface-only'],
                   help="Interior propagation of the prescribed "
                   "surface increment. 'diffuser' (default): Poisson "
                   "harmonic extension (original). 'winslow': move "
                   "surface vertices directly + graph-Laplacian "
                   "relax the interior (no Poisson solve).")
    p.add_argument('--winslow-deform-iters', type=int, default=20,
                   help="Winslow sweeps per deform_by_inc call when "
                   "--deform-mode winslow (default 20).")
    ...
    # Interior-deform-propagation strategy (read by deform_by_inc).
    globals()['_DEFORM_MODE'] = args.deform_mode
    globals()['_WINSLOW_DEFORM_ITERS'] = args.winslow_deform_iters
    ...
    for scheme in schemes:
        ...
        state = _build(res=args.res, Ra=args.Ra,
                       ...
                       diffusion_theta=float(args.diffusion_theta),
                       monotone_mode=args.monotone_mode)
        adv_diff = state['adv_diff']
        ...
        for s in range(args.n_steps):
            ...
            if (args.winslow_iters > 0
                ...
                impl = getattr(args, 'smoother_impl', 'uw')
                if impl == 'in-script':
                    ...
                    # uw3 parallel-safe smoother (PETSc Mat assembly,
                    # partition-independent to 1 ULP). Pins every
                    # named boundary by default.
                    import underworld3 as uw
                    uw.meshing.smooth_mesh_interior(
                        state['mesh'],
                        n_iters=args.winslow_iters,
                        alpha=args.winslow_alpha)
            # Per-step mesh-coord dump (for the smoother comparison
            # plots — only fires when --dump-mesh-dir is set).
            if getattr(args, 'dump_mesh_dir', None):
                _dd = args.dump_mesh_dir
                os.makedirs(_dd, exist_ok=True)
                np.savez(
                    os.path.join(_dd, f"step_{s + 1:04d}.npz"),
                    coords=np.asarray(state['mesh'].X.coords),
                    T=np.asarray(state['t_soln'].data[:, 0]),
                    T_coords=np.asarray(state['t_soln'].coords),
                    step=s + 1)

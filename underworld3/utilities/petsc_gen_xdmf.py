    ...
    def __init__(self, filename):
        ...
        if sys.version_info[0] < 3:
            ...
            self.typeExt = {
                ...
                3: {b"vector": ["x", "y", "z"], b"tensor": ["xx", "yy", "zz", "xy", "yz", "xz"]},
            }
        return
    ...
    def write(
        ...
        with open(self.filename, "w") as fp:
            ...
            for t in range(len(time)):
                ...
                for vf in vfields:
                    self.writeField(fp, len(time), t, cellDim, spaceDim, "/vertex_fields/" + vf[0], vf, "Node")
                for cf in cfields:
                    self.writeField(fp, len(time), t, cellDim, spaceDim, "/cell_fields/" + cf[0], cf, "Cell")
                self.writeSpaceGridFooter(fp)
                if numHCells:
                    self.writeSpaceGridFooter(fp)
            if useTime:

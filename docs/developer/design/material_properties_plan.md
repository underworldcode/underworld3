...
## Symbol Uniqueness Integration with Materials Registry

### Current Implementation (Interim Solution)
The constitutive model system currently uses automatic instance-based numbering for symbol uniqueness:
- First instance: Clean symbol (e.g., `η`)
- Subsequent instances: Superscript numbering (e.g., `{η}^{(1)}`, `{η}^{(2)}`)
- 0-based indexing: First instance gets index 0 (clean symbol), second gets index 1

### Proposed Materials Registry Integration
When the materials registry is implemented, constitutive model symbol uniqueness should transition to use material-based indexing:

```python
# Current automatic numbering
material_0 = ViscousFlowModel(unknowns)  # η
material_1 = ViscousFlowModel(unknowns)  # {η}^{(1)}

# Proposed materials registry integration
mantle = Material("mantle")
crust = Material("crust")

material_mantle = mantle.create_constitutive_model(ViscousFlowModel, unknowns)  # η_mantle
material_crust = crust.create_constitutive_model(ViscousFlowModel, unknowns)   # η_crust

# Or with superscript notation
material_mantle = mantle.create_constitutive_model(ViscousFlowModel, unknowns)  # {η}^{(mantle)}
material_crust = crust.create_constitutive_model(ViscousFlowModel, unknowns)   # {η}^{(crust)}
```

### Implementation Approach
1. **Extend `create_unique_symbol()` method**: Add optional material_id parameter
2. **Materials registry provides unique IDs**: Each registered material gets a unique identifier
3. **Backward compatibility**: If no material_id provided, fall back to current instance numbering
4. **User control**: Allow choice between subscript (`η_mantle`) vs superscript (`{η}^{(mantle)}`) notation

### Benefits
- **Semantic clarity**: Symbols reflect actual physical materials, not arbitrary instance order
- **Reproducibility**: Same materials always get same symbols across sessions
- **User intent**: Symbols match user's mental model of material organization
- **Multi-physics consistency**: Same material symbols across thermal, mechanical, chemical models
...
6. **Symbol Uniqueness Integration**: Automatic material-based symbol generation
...
6. **Meaningful Symbols**: Constitutive model symbols reflect actual material names
...
5. **Symbol Management**: Material-based constitutive model symbol uniqueness

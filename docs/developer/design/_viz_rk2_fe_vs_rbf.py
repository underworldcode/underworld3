"""Side-by-side RK2 FE-traceback (rings) vs RK2 RBF-traceback (clean)
for steps 28-35. Top row: FE; bottom row: RBF.
"""
import os
import numpy as np
...
FE_DIR  = "output/convection_zoo_snapshots_rk2_full"
RBF_DIR = "output/convection_zoo_snapshots_rk2_rbf_traceback"
STEPS = [28, 31, 33, 34, 35]
OUT_PNG = "output/rk2_fe_vs_rbf_comparison.png"
...
for row, (snap_dir, label) in enumerate(
        [(FE_DIR, "RK2 + FE trace-back (default)"),
         (RBF_DIR, "RK2 + RBF trace-back (fix)")]):

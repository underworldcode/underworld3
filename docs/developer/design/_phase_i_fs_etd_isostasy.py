...
# Sign of the stress-derived equilibrium topography h_eq = SIGN·σ_rr/(Δρg).
# Determined empirically at the first step (see the [etd_topo] debug print):
# the pole h_eq must match the rk4 equilibrium (+0.0268 here). σ = stress =
# τ - pI, outward radial traction r̂·σ·r̂ lifts the surface, so SIGN = +1.
ETD_TOPO_SIGN = 1.0
...
def run(scheme, dt_factor, n_steps, res=20, blob_amp=0.5,
    ...
    for step in range(n_steps):
        ...
        if update == 'fe':
            ...
            # Stress-equilibrium exponential integrator.
            # h_eq is measured directly from the stress field (the dynamic
            # topography the surface relaxes toward); the surface is then
            # integrated exponentially from h^n to that KNOWN end-state:
            #   h^{n+1} = h_eq + (h^n - h_eq) e^{-γΔt}
            # γ is inverted from the linear-response pair (h_eq - h, u_n),
            # which is well-defined even at flat IC because h_eq (from
            # stress) is non-zero — the cure for the curvS/empE flat-IC
            # failure where curvature(h≈0) is pure noise.
            dt_val = float(delta_t.value)
            ...
            # 1. h_eq from the radial normal stress σ_rr = r̂·σ·r̂.
            #    Δρ·g = 1 here (heavy ρ=1 / air ρ=0 across r_o, g=1).
            sigma = stokes.stress
            sigma_rr_expr = (unit_r * sigma * unit_r.T)[0, 0]
            sigma_rr = np.asarray(uw.function.evaluate(
                sigma_rr_expr,
                mesh.X.coords[internal_idx])).flatten()
            drho_g = 1.0
            h_eq = ETD_TOPO_SIGN * sigma_rr / drho_g

            # 2. Per-node relaxation rate γ_i = u_n_i / (h_eq_i - h_i).
            #    ḣ = -γ(h - h_eq) ⇒ u_n = γ(h_eq - h), so the local rate
            #    is just u_n / disp. Clamp γ ≥ 0 (no anti-relaxation):
            #    where u_n opposes disp the linear model is locally
            #    invalid (noisy h_eq) — freeze rather than move spuriously.
            #    This also suppresses the off-pole σ_rr noise that
            #    corrupts a single global γ.
            ...
            with np.errstate(divide='ignore', invalid='ignore'):
                gamma = np.where(np.abs(disp) > eps_disp,
                                 u_n / disp, 0.0)
            gamma = np.maximum(gamma, 0.0)

            if verbose:
                ip = pole_idx_in_internal
                print(f"    [etd_topo] h_eq_pole={h_eq[ip]:+.4e} "
                      f"h_pole={upper_dr[ip]:+.4e} "
                      f"u_n_pole={u_n[ip]:+.4e} "
                      f"γ_pole={gamma[ip]:.4e}", flush=True)

            # 3. exponential step toward the measured end-state.
            #    increment = disp·(1 - e^{-γΔt}); with γ≥0 this is in
            #    [0, disp] (same sign as disp) — cannot overshoot h_eq.
            increment = disp * (1.0 - np.exp(-dt_val * gamma))
            upper_dr_new = upper_dr + increment
...
def main():
    ...
    if args.scheme == 'all':
        ...
        run_list = [schemes['fe-nofssa'], schemes['rk2-nofssa'],
                    ...
                    schemes['midpoint-fssa'], schemes['etd_topo-nofssa']]
    elif args.scheme == 'etd-vs-rk4':
        # Head-to-head for the stress-equilibrium integrator: the
        # accuracy winner (rk4), the saturation fallback (curvS), and
        # the new stress-equilibrium scheme (FSSA on/off).
        run_list = [schemes['rk4-nofssa'], schemes['curvS-fssa'],
                    schemes['etd_topo-nofssa'], schemes['etd_topo-fssa']]
    else:

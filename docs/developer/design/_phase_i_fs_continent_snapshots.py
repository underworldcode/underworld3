...
OUT_DIR = "output"
SNAP_DIR = os.path.join(OUT_DIR, "continent_snapshots")
...
def _build(res=20, beta=0.2, theta_block_half=0.4, r_min=0.7,
    ...
    if structured:
        ...
        mesh = AnnulusInternalBoundaryStructured(
            radiusOuter=r_outer, radiusInternal=r_o,
            radiusInner=r_inner,
            nRadialInner=nRi, nRadialOuter=nRo, nAngular=nA,
            qdegree=3,
        )
    else:
        mesh = uw.meshing.AnnulusInternalBoundary(
            radiusOuter=r_outer, radiusInternal=r_o,
            radiusInner=r_inner,
            cellSize_Outer=3.0 * cellsize, cellSize=cellsize,
            qdegree=3,
        )
    r, th = mesh.CoordinateSystem.R

    Vr = uw.discretisation.MeshVariable(
        f"Vr_cont_snap", mesh, vtype=uw.VarType.SCALAR, degree=1,
        continuous=True, varsymbol=r"v_r")
    v = uw.discretisation.MeshVariable(
        f"V_cont_snap", mesh, vtype=uw.VarType.VECTOR, degree=2,
        continuous=True, varsymbol=r"\mathbf{v}")
    p = uw.discretisation.MeshVariable(
        f"P_cont_snap", mesh, vtype=uw.VarType.SCALAR, degree=1,
        continuous=True, varsymbol="p")
    M = uw.discretisation.MeshVariable(
        f"M_cont_snap", mesh, vtype=uw.VarType.SCALAR, degree=0,
        continuous=False, varsymbol=r"\rho")
    B = uw.discretisation.MeshVariable(
        f"B_cont_snap", mesh, vtype=uw.VarType.SCALAR, degree=0,
    ...
    # Lagrangian block: 1 in sector |θ|<θ_b AND r∈(r_min, r_o]
    ...
    stokes.penalty = 0.0
    # Full buoyancy with Lagrangian sector block: ambient rock has
    # density 1, block has density (1−β), air has density 0.
    stokes.bodyforce = (
...
def _capture(state, scheme_name, label_short, h_pole_value,
    ...
    Writes:
      1. UW3-native checkpoint via mesh.write_timestep (H5 + XDMF) —
         the deformed mesh + M, B variables.
      2. A pyvista VTU of the thresholded heavy-layer pvmesh with
         cell_data: M, B, and eff = M − β·B (effective density).
      3. A profile npz with internal-boundary (θ, δr).
    """
    import underworld3.visualisation as vis
    mesh = state['mesh']; M = state['M']; B = state['B']
...
def _plot_grid(schemes, labels, h_eq, suffix=""):
    ...
    out = os.path.join(
        OUT_DIR, f"phase_i2d_fs_continent_snapshots{suffix}.png")
    pl.screenshot(out)
    pl.close()
...
def _plot_profiles(schemes, labels, suffix=""):
    ...
    fig.tight_layout()
    out = os.path.join(
        OUT_DIR, f"phase_i2d_fs_continent_profiles{suffix}.png")
...
def main():
    ...
    global SNAP_DIR
    if args.structured:
        SNAP_DIR = os.path.join(OUT_DIR, "continent_snapshots_struct")

...
with swarm.access(material, strain_p), mesh1.access():
    strain_p.data[:] = strain_rate_inv2_p.rbf_interpolate(swarm._particle_coordinates.data)
    strain_array = strain_rate_inv2_p.rbf_interpolate(swarm._particle_coordinates.data)
# +
...
with swarm.access(strain_p, material), mesh1.access():
    ...
    strain_p.data[:,0] = 0.0 * np.random.random(swarm._particle_coordinates.data.shape[0]) * mask
# -

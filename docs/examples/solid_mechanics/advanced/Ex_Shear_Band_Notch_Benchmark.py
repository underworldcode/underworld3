...
visc = uw.discretisation.MeshVariable(r"\eta", mesh1, 1, degree=0, continuous=False)
...
stokes.petsc_options["fieldsplit_pressure_pc_type"] = (
    "gamg"  # can use gasm / gamg / lu here
)
stokes.petsc_options["fieldsplit_pressure_pc_gasm_type"] = (
    "basic"  # can use gasm / gamg / lu here
)
stokes.petsc_options["fieldsplit_pressure_pc_gamg_type"] = (
...
stokes.add_dirichlet_bc((1.0, 0), "Left")
stokes.add_dirichlet_bc((-1.0, 0), "Right")
...
for i in range(1, 10, 2):
    ...
    uw.pprint(f"Completed: Mu - {mu}, C = {C}")
0 / 0
...
mesh1.petsc_save_checkpoint(
    index=0, meshVars=[p_soln, v_soln, edot], outputPath=savefile
)
...
if uw.mpi.size == 1:
    ...
    pvmesh.cell_data["visc"] = visc.array[:, 0, 0]
    ...
    pl.add_arrows(
        velocity_points.points, velocity_points.point_data["V"], mag=0.02, opacity=0.75
    )

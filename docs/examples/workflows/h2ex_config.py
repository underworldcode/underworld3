...
def _solve_stokes_stress(mesh, surfaces, config, fault_weakness_value, prefix=""):
    ...
    prefix : str
        Prefix for variable names to avoid collisions when solving
    ...
    # Variables (prefix avoids name collisions on shared mesh)
    v = uw.discretisation.MeshVariable(f"{prefix}U", mesh, mesh.dim, degree=2)
    p = uw.discretisation.MeshVariable(f"{prefix}P", mesh, 1, degree=1)
    strain_rate_inv2 = uw.discretisation.MeshVariable(f"{prefix}eps", mesh, 1, degree=1)

    # Stokes solver — use anisotropic model only when faults are present
    stokes = uw.systems.Stokes(mesh, velocityField=v, pressureField=p)
    stokes.penalty = 0.1

    if surfaces is not None and len(surfaces.surfaces) > 0:
        # TransverseIsotropic: weak along fault planes
    ...
    for bnd_name in ["West", "North", "East", "South", "Bottom"]:
...
def create_mesh(config: H2ExConfig):
    ...
    depth_range = (
        uw.quantity(config.depth_range_km[0], "km"),
        uw.quantity(config.depth_range_km[1], "km"),
    )

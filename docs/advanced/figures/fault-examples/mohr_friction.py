...
def draw_stress_plane(ax):
    ...
    ss = np.linspace(-1.35 * R_ANALYTIC, 0, 50)
    for sgn in (+1, -1):
        ax.plot(ss, sgn * MU * (-ss), "--", color="0.35", lw=1.0,
                label=(r"friction envelope $\tau = \pm\mu|\sigma_n|$"
                       if sgn > 0 else None))
...
ax.plot(probes[~sliding, 1], probes[~sliding, 2], "o", ms=7,
        color="#c62828", label="stuck: on the circle", zorder=5)
ax.plot(probes[sliding, 1], probes[sliding, 2], "s", ms=6,
...
for k in range(len(probes)):
    ...
    axr.plot(probes[:k + 1][stuck_prev, 1], probes[:k + 1][stuck_prev, 2],
             "o", ms=5, mfc="none", mec="#c62828", mew=1.2)
    axr.plot(probes[:k + 1][slide_prev, 1], probes[:k + 1][slide_prev, 2],

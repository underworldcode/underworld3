...
for step in range(nsteps):
    ...
    stokes.solve()
    uw.pprint(0, f"Step {step}: dt={dt:.3e}, time={time:.3e}")
...
uw.pprint(0, f"Velocity max: {velocity_stats['max']:.6e}")
uw.pprint(0, f"Pressure range: [{pressure_stats['min']:.6e}, {pressure_stats['max']:.6e}]")
```
...
uw.pprint(0, f"Result: {var.stats()}")
```
...
# NEW: Use pprint
uw.pprint(0, f"Iteration {step}, Time: {time}")
```
...
uw.pprint(0, f"Max: {var.stats()['max']}")
```
...
| Task | Use | Example |
|------|-----|---------|
| Print from specific ranks | `uw.pprint(ranks, ...)` | `uw.pprint(0, "Done")` |
| Serial code (viz, I/O) | `with uw.selective_ranks(ranks):` | Matplotlib, file writes |
| Collective with output | `uw.pprint()` with collective args | `uw.pprint(0, var.stats())` |
...
|--------|---------|---------|
| `0` | Single rank | `uw.pprint(0, "message")` |
...
- [ ] Replace `if uw.mpi.rank == 0: print(...)` with `uw.pprint(0, ...)`

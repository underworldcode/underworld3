...
class Run:
    ...
    def append_step(
        ...
        mesh_updates : bool, default False
            Pass ``True`` for an **adaptive / moving mesh** so the mesh
            geometry is checkpointed *per step* (a separate
            ``run.mesh.NNNNN.h5`` each save).  The default (``False``)
            writes the geometry once and shares it across steps — correct
            only for a static mesh; on a deforming mesh it would pair
            later fields with stale step-0 geometry.
        """
        mesh.write_timestep(
            filename=RUN_NAME,
            index=step,

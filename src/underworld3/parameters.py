    ...
    def validate(self, value: Any) -> bool:
        ...
        if self.ptype == ParameterType.SCALAR:
            ...
            if self.bounds and not (self.bounds[0] <= value <= self.bounds[1]):
                raise ValueError(f"Parameter '{self.name}' must be in range {self.bounds}, got {value}")
...
def define_thermal_convection_parameters(registry: ParameterRegistry) -> None:
    ...
    registry.define_parameter(
        'Ra', ParameterType.SCALAR, default=1e5, bounds=(1e3, 1e8),
        description='Rayleigh number', units='dimensionless'
    )
    
    registry.define_parameter(
        'material.viscosity', ParameterType.SCALAR, default=1.0, bounds=(1e-6, 1e6),
        description='Reference viscosity', units='Pa·s'
    )
    
    registry.define_parameter(
        'material.thermal_diffusivity', ParameterType.SCALAR, default=1e-6, bounds=(1e-9, 1e-3),
        description='Thermal diffusivity', units='m²/s'
    )
    
    registry.define_parameter(
        'solver.tolerance', ParameterType.SCALAR, default=1e-6, bounds=(1e-12, 1e-3),
        description='Solver convergence tolerance', units='dimensionless'
    )
    
    registry.define_parameter(
        'solver.max_iterations', ParameterType.INTEGER, default=1000, bounds=(10, 10000),
        description='Maximum solver iterations', units='count'
    )
    
    registry.define_parameter(
        'mesh.resolution', ParameterType.ARRAY, default=[32, 32],
        description='Mesh resolution in each dimension', units='elements'
    )
...
def define_stokes_flow_parameters(registry: ParameterRegistry) -> None:
    ...
    registry.define_parameter(
        'viscosity_contrast', ParameterType.SCALAR, default=1e3, bounds=(1, 1e8),
        description='Viscosity contrast between materials', units='dimensionless'
    )
    
    registry.define_parameter(
        'boundary_conditions.velocity.top', ParameterType.ENUM, 
        choices=['free_slip', 'no_slip', 'prescribed'], default='free_slip',
        description='Top boundary velocity condition'
    )
    
    registry.define_parameter(
        'boundary_conditions.velocity.bottom', ParameterType.ENUM,
        choices=['free_slip', 'no_slip', 'prescribed'], default='no_slip', 

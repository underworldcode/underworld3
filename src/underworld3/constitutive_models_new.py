r"""
Alternative constitutive model implementations.

.. deprecated::
    This module contains a failed attempt to rebuild the constitutive models
    and is scheduled for removal. Use :mod:`underworld3.constitutive_models`
    instead for all constitutive model needs.

See Also
--------
underworld3.constitutive_models : Primary constitutive model module (use this).
"""
...
class ViscousFlowModel(Constitutive_Model):
    ...
    def _object_viewer(self):
        ...
        display(Latex(r"$\quad\eta = $ " + sympy.sympify(self.Parameters.viscosity)._repr_latex_()))
            ...
            if inner_self.materialIndex == None:
                ...
                if yield_stress_fn == sympy.oo or inner_self.strainrate_inv_II == sympy.oo:
                    ...
                    pl_effective_viscosity = yield_stress_fn / (
                        (2 * inner_self.strainrate_inv_II) + inner_self.strainrate_inv_II_min
                    )
                ...
                if len(inner_self.yield_stress) != inner_self.materialIndex.indices:
                    ...
                    inner_self.yield_stress = list(
                        np.repeat(inner_self.yield_stress[0], inner_self.materialIndex.indices)
                    )
                    ...
                    if inner_self.averaging_method.casefold() == "min":
                        if yield_visc[i] != 0:
                            ...
                            viscosity_list.append(
                                sympy.Min(
                                    shear_viscosity_max[i],
                                    sympy.Max(shear_viscosity_min[i], shear_viscosity_0[i]),
                                )
                            )
                    else:
                        if yield_visc[i] != 0:
                            viscosity_list.append(
                                sympy.Min(
                                    ...
                                    sympy.Max(
                                        shear_viscosity_min[i],
                                        1 / ((1 / shear_viscosity_0[i]) + (1 / yield_visc[i])),
                                    ),
                                )
                            )
                        else:
...
class ViscoElasticPlasticFlowModel(Constitutive_Model):
            ...
            if inner_self.materialIndex == None:
                ...
                if len(inner_self.dt_elastic) != inner_self.materialIndex.indices:
                    ...
                    inner_self.dt_elastic = list(
                        np.repeat(inner_self.dt_elastic[0], inner_self.materialIndex.indices)
                    )
            ...
            if inner_self.materialIndex == None:
                ...
                if inner_self.averaging_method.casefold() == "min":
                    if yield_visc != 0:
                        effective_viscosity = sympy.Min(
                            shear_viscosity_max,
                            sympy.Max(shear_viscosity_min, sympy.Min(yield_visc, ve_eff_visc)),
                        )
                    else:
                        effective_viscosity = sympy.Min(
                            shear_viscosity_max,
                        ...
                        if yield_visc[i] != 0:
                            viscosity_list.append(
                                sympy.Min(
                                    ...
                                    sympy.Max(
                                        shear_viscosity_min[i],
                                        1 / ((1 / ve_eff_visc[i]) + (1 / yield_visc[i])),
                                    ),
                                )
                            )
                        else:
    ...
    def _object_viewer(self):
        ...
        display(
            Latex(
                r"$\quad\mu = $ " + sympy.sympify(self.Parameters.shear_modulus[0])._repr_latex_(),
            ),
            Latex(
                r"$\quad\Delta t_e = $ "
            ...
            Latex(
                r"$\quad \sigma^* = $ " + sympy.sympify(self.Parameters.stress_star)._repr_latex_(),
            ),
        )
...
class TransverseIsotropicFlowModel(Constitutive_Model):
    r"""
    Transversely isotropic viscous flow model.
...
class MultiMaterial(Constitutive_Model):
    r"""

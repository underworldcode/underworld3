...
def swarm_h5(
    swarm, fileName, timestep, fields=None, outputPath="", compression=False, compressionType="gzip"
):
    """
    ...
    if fields != None and not isinstance(fields, list):
        ...
        swarm.save(
            filename=f"{outputPath}{fileName}-{timestep:04d}.h5",
        ...
        if fields != None:
            for field in fields:
                field.save(
                    filename=f"{outputPath}{field.name}-{timestep:04d}.h5",
                    compression=compression,
                    compressionType=compressionType,
                )
...
def swarm_xdmf(fileName, timestep, fields=None, outputPath="", time=None):
    ...
    if fields != None and not isinstance(fields, list):
        raise RuntimeError(
            "`swarm_xdmf()` function parameter `fields` does not appear to be a list."
        )
    ...
    if fields != None:
        for field in fields:
            if not os.path.exists(f"{outputPath}{field.name}-{timestep:04d}.h5"):
                raise RuntimeError(
                    f"`swarm_xdmf()` could not find '{field.name}-{timestep:04d}.h5'."
                )

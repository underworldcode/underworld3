...
def boundary_normal_traction(solver, boundary, solve_result, mass="auto"):
    ...
    ``mass`` selects the de-smear:
      * ``"auto"`` (default) — lumped for 2D traces and 3D P1 triangles, P1-PROJECTED
        for 3D P2 triangles.
      ...
      * ``"p1"`` — P1-PROJECTED recovery on a 3D P2 trace (edge-midpoint loads folded
        onto vertices, lumped P1 triangle mass). Monotone, and the ``"auto"`` choice on
        a 3D P2 trace since #633. On a P1 trace, identical to ``"lumped"``.
...
def dynamic_topography_field(solver, boundary, solve_result, field,
    ...
    lumped recovery where valid and P1-projected recovery for 3D P2 triangles.

...
def boundary_normal_traction_integral(solver, boundary, solve_result, fn,
    ...
    Each assembled reaction DOF is counted on its owning rank only, followed by
    an MPI sum.  The operation is distributed and does not gather boundary
    topology or recovered values onto rank zero.  ``remove_mean=True`` removes
    the constant-traction gauge using boundary integrals of ``fn`` and one.
    """

...
outputPath = f'./output/1dblock_adv_diff_supg_dcterm/'
...
adv_diff = AdvDiffusionSUPG(
    mesh, u_Field=T, V_fn=v,
    diffusivity=0.0,
    discontinuity_capturing=True,
    dc_coefficient=0.2,
    dc_streamwise_weight=0.3,   # was: dc_crosswind_only=False (i.e. weight=1.0)
)
...
with mesh.access(T):
    T.data[:,0] = uw.function.evaluate(Ts0, T.coords)[:,0,0]
...
for step in range(0, steps):
    mesh.write_timestep("mesh", meshUpdates=False, meshVars=[T],
...
if uw.mpi.size == 1:
        ...
        pl.show(cpos="xy",screenshot=outputPath+"output.png")

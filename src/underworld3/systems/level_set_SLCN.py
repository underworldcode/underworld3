...
# =============================================================================
# Reinitialisation gradient: 2nd-order ENO + Godunov upwinding
# (Osher & Shu, 1991; Jiang & Peng, 2000; Sussman, Smereka & Osher, 1994)
# =============================================================================
def _snap_to_grid_indices(vals: np.ndarray, rtol: float):
    """Map coordinates that should form an evenly-spaced 1D grid (possibly
    with small floating-point noise between nominally-equal values) onto
    integer grid indices, robustly.

    Deliberately NOT a sequential/chain tolerance-merge (compare each
    sorted value only to the previous *accepted* one): that construction
    is fragile by design -- A within tol of B and B within tol of C does
    not imply A is within tol of C, so a slow "creep" across several
    values can chain-merge a run that should have been split into
    distinct grid lines, or (as observed in practice, on a real Q2 mesh
    where shared-vertex DOF coordinates are computed via different
    elements' local coordinate maps and needn't be bit-identical) merge
    inconsistently depending on value order, breaking the index mapping.

    Instead: estimate the true grid spacing from the smallest gap between
    sorted *unique* values that's clearly not just noise (bigger than
    ``rtol`` times the value range), then snap every value to the nearest
    integer multiple of that spacing from a single fixed reference (the
    minimum value) -- every value is compared against one global
    reference, not against its neighbours in a chain, so there's no
    order-dependent failure mode.

    Returns (indices, spacing, origin).
    """
    uniq = np.unique(vals)
    if uniq.size == 1:
        return np.zeros(vals.shape, dtype=int), 1.0, float(uniq[0])

    span = uniq[-1] - uniq[0]
    noise_floor = rtol * max(span, 1.0)
    gaps = np.diff(uniq)
    significant = gaps[gaps > noise_floor]
    if significant.size == 0:
        raise RuntimeError(
            "_snap_to_grid_indices: no gap between distinct coordinate "
            f"values exceeds the noise floor ({noise_floor:.3e}, from "
            f"rtol={rtol:.1e} x span={span:.3e}) -- either every point is "
            "genuinely coincident, or rtol needs loosening for this mesh's "
            "coordinate scale."
        )
    spacing = float(np.min(significant))
    origin = float(uniq[0])

    indices = np.round((vals - origin) / spacing).astype(int)
    return indices, spacing, origin


class _StructuredGrid:
    """Maps a continuous Lagrange field's DOFs on a structured
    (Cartesian-topology) quad mesh to a regular ``(ny, nx)`` array of
    nodes, so classical Cartesian finite-difference schemes -- here,
    2nd-order ENO -- can be applied via plain array indexing instead of
    unstructured per-cell stencils.

    Built once from ``var.coords`` via :func:`_snap_to_grid_indices`
    (snap-to-nearest-multiple-of-estimated-spacing from a fixed
    reference, robust to floating-point noise between nominally-equal
    shared-vertex coordinates -- see that function's docstring for why a
    naive sequential tolerance merge is NOT used here), agnostic to
    whatever internal DOF ordering UW3 happens to use, as long as the
    node set genuinely forms a regular grid (true for any Lagrange degree
    on ``uw.meshing.StructuredQuadBox``; raises ``RuntimeError`` rather
    than a silently-wrong mapping if it does not, e.g. on an unstructured
    or simplex mesh).

    **Serial only.** ``var.coords`` is the *rank-local* DOF set; a
    parallel-consistent version would need an allgather of coordinates and
    a halo exchange for the two ghost points ENO needs at every
    partition boundary. Not implemented here -- run this on a single rank,
    or extend this class before trusting it in parallel.
    """

    def __init__(self, var: uw.discretisation.MeshVariable, rtol: float = 1.0e-6):
        coords = np.asarray(var.coords)
        if coords.shape[1] != 2:
            raise NotImplementedError("_StructuredGrid currently implements 2D only.")
        x, y = coords[:, 0], coords[:, 1]

        ix, self.dx, self._x0 = _snap_to_grid_indices(x, rtol)
        iy, self.dy, self._y0 = _snap_to_grid_indices(y, rtol)

        self.nx = int(ix.max()) + 1
        self.ny = int(iy.max()) + 1
        if self.nx * self.ny != coords.shape[0]:
            raise RuntimeError(
                f"_StructuredGrid: {coords.shape[0]} DOFs do not factor into "
                f"a {self.ny} x {self.nx} regular grid ({self.ny * self.nx} "
                "expected) -- this mesh/field does not have a genuinely "
                "structured (Cartesian-topology) node layout, or `rtol` "
                "needs adjusting for its coordinate noise/spacing scale "
                f"(dx={self.dx:.3e}, dy={self.dy:.3e} were the estimated "
                "spacings)."
            )

        flat_idx = iy * self.nx + ix
        if np.unique(flat_idx).size != coords.shape[0]:
            raise RuntimeError(
                "_StructuredGrid: DOF-to-grid-index mapping is not one-to-one "
                f"even though nx*ny matched (nx={self.nx}, ny={self.ny}, "
                f"dx={self.dx:.3e}, dy={self.dy:.3e}) -- try loosening/"
                "tightening `rtol` for this mesh's actual coordinate noise "
                "scale."
            )

        self._i, self._j = ix, iy

    def to_grid(self, flat_array: np.ndarray) -> np.ndarray:
        grid = np.empty((self.ny, self.nx))
        grid[self._j, self._i] = flat_array
        return grid

    def to_dofs(self, grid_array: np.ndarray) -> np.ndarray:
        return grid_array[self._j, self._i]


def _minmod(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    """Standard two-argument minmod: same sign as both, magnitude the
    smaller of the two -- zero if they disagree in sign."""
    same_sign = np.sign(a) == np.sign(b)
    return np.where(same_sign, np.sign(a) * np.minimum(np.abs(a), np.abs(b)), 0.0)


def _eno2_one_sided(f: np.ndarray, h: float, axis: int):
    """Second-order ENO one-sided derivatives (Osher & Shu, 1991) of a
    regular-grid array ``f`` along ``axis``, spacing ``h``.

    Classic HJ-ENO2 construction: start from the first-order one-sided
    (upwind) difference, then correct it with the smaller-magnitude
    (same-sign) of the two neighbouring second-difference estimates --
    i.e. pick whichever of the two candidate quadratic stencils is
    smoother, so the reconstruction doesn't differ across a kink in the
    field. Returns ``(D_minus, D_plus)``, each the same shape as ``f``.

    The two ghost points ENO needs on each side of the domain are filled
    by constant (edge-value) extrapolation -- a simple, standard choice
    that approximates a zero-gradient/Neumann boundary, consistent with
    this module's existing ``_apply_boundary_neumann`` treatment
    elsewhere.
    """
    pad_width = [(0, 0)] * f.ndim
    pad_width[axis] = (2, 2)
    fp = np.pad(f, pad_width, mode="edge")

    n = f.shape[axis]

    def shift(k):
        sl_ = [slice(None)] * f.ndim
        sl_[axis] = slice(2 + k, 2 + k + n)
        return fp[tuple(sl_)]

    fm2, fm1, f0, fp1, fp2 = shift(-2), shift(-1), shift(0), shift(1), shift(2)

    D1_mh = (f0 - fm1) / h    # D_{i-1/2}
    D1_ph = (fp1 - f0) / h    # D_{i+1/2}
    D1_m3h = (fm1 - fm2) / h  # D_{i-3/2}
    D1_p3h = (fp2 - fp1) / h  # D_{i+3/2}

    D2_im1 = (D1_mh - D1_m3h) / h
    D2_i = (D1_ph - D1_mh) / h
    D2_ip1 = (D1_p3h - D1_ph) / h

    D_minus = D1_mh + (h / 2.0) * _minmod(D2_im1, D2_i)
    D_plus = D1_ph - (h / 2.0) * _minmod(D2_i, D2_ip1)
    return D_minus, D_plus


def _grad_magnitude_eno2(phi: np.ndarray, phi0_sign: np.ndarray, dx: float, dy: float) -> np.ndarray:
    """``|grad phi|`` via 2nd-order ENO one-sided differences (Osher & Shu,
    1991; Jiang & Peng, 2000) combined with Godunov's upwind selection
    based on the sign of the *frozen* reference field ``phi0_sign``
    (Sussman, Smereka & Osher, 1994) -- the standard, stable numerical
    Hamiltonian for the reinitialisation equation's gradient term.

    ``phi0_sign`` should be ``sign(phi0 - 0.5)`` for a CLS field in
    [0, 1] (interface at 0.5), computed *once* at the start of a
    reinitialisation call and held fixed through all of its pseudo-time
    stages -- re-deriving the sign from the evolving field at every stage
    would let the upwind choice itself drift as the profile sharpens,
    which is exactly the kind of inconsistency Godunov upwinding is meant
    to avoid.
    """
    Dxm, Dxp = _eno2_one_sided(phi, dx, axis=1)  # x varies along columns
    Dym, Dyp = _eno2_one_sided(phi, dy, axis=0)  # y varies along rows

    pos = phi0_sign > 0
    ax = np.where(pos, np.maximum(Dxm, 0.0), np.minimum(Dxm, 0.0))
    bx = np.where(pos, np.minimum(Dxp, 0.0), np.maximum(Dxp, 0.0))
    ay = np.where(pos, np.maximum(Dym, 0.0), np.minimum(Dym, 0.0))
    by = np.where(pos, np.minimum(Dyp, 0.0), np.maximum(Dyp, 0.0))

    gx2 = np.maximum(ax ** 2, bx ** 2)
    gy2 = np.maximum(ay ** 2, by ** 2)
...
class LevelSetSolver:
    ...
    def __init__(
        ...
        *,
        ...
        # ---- Advection solver (SLCN, zero diffusivity) --------------------
        ...
        self._comp_ddt = uw.systems.ddt.SemiLagrangian(
            ...
            varsymbol="cphi", bcs=[], order=1, smoothing=0.0,
        ...
        self._adv_solver = systems.AdvDiffusionSLCN(
            self.mesh,
            u_Field=self.phi,
            V_fn=self.velocity,order=1, DuDt=self._comp_ddt,
        )
        # Zero diffusivity → pure advection
        self._adv_solver.constitutive_model = uw.constitutive_models.DiffusionModel
        self._adv_solver.constitutive_model.Parameters.diffusivity = 0.
        self._adv_solver.tolerance = 1.0e-4
        self._adv_solver_bc = adv_solver_bc  

        # ---- Reinitialisation: ENO2/Godunov gradient on a regular grid ----
    ...
    def reinitialise(self) -> None:
        ...
        """
        phi0_grid = self._grid.to_grid(self.phi.data[:, 0])
        self._phi0_sign_grid = np.sign(phi0_grid - 0.5)
    ...
    def _rhs(self, phi_values: np.ndarray) -> np.ndarray:
        ...
        Reshapes `phi_values` onto the regular node grid, computes
        |grad phi| there via ENO2 + Godunov upwinding against the frozen
        sign reference from `reinitialise()`, then reshapes back and
        combines the sharpening and balancing terms nodally:

          sharpening = −φ(1−φ)(1−2φ)      balance = ε(1−2φ)|∇φ|
        """
        phi_grid = self._grid.to_grid(phi_values)
        gmag_grid = _grad_magnitude_eno2(
            phi_grid, self._phi0_sign_grid, self._grid.dx, self._grid.dy
        )
        grad = self._grid.to_dofs(gmag_grid)

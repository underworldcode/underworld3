...
@timing.routine_timer_decorator
def StructuredQuadBoxInternalBoundary2D(
    elementRes: Optional[Tuple[int, int]] = (16, 16),
    yelementRes: Optional[Tuple[int, int]] = (6, 10),
    minCoords: Optional[Tuple[float, float]] = (0, 1),
    maxCoords: Optional[Tuple[float, float]] = (0, 1),
    yintCoord: float = 0.5,
    degree: int = 1,
    qdegree: int = 2,
    filename=None,
    ...
    if filename is None:
        if uw.mpi.rank == 0:
            os.makedirs(".meshes", exist_ok=True)
        uw_filename = (
            f".meshes/uw_structuredQuadBoxIB_minC{minCoords}_maxC{maxCoords}.msh"
        )
    else:
        uw_filename = filename
    ...
    if uw.mpi.rank == 0:
        ...
        yint = yintCoord
        nx,ny = elementRes
        ny_a,ny_b = yelementRes

        p1 = gmsh.model.geo.add_point(xmin, ymin, 0.0)
        p2 = gmsh.model.geo.add_point(xmax, ymin, 0.0)
        p3 = gmsh.model.geo.add_point(xmin, ymax, 0.0)
        p4 = gmsh.model.geo.add_point(xmax, ymax, 0.0)
        p5 = gmsh.model.geo.add_point(xmin, yint, 0.0)
        p6 = gmsh.model.geo.add_point(xmax, yint, 0.0)
...
@timing.routine_timer_decorator
def StructuredQuadBoxInternalBoundary3D(
    elementRes: Optional[Tuple[int, int, int]] = (16, 16, 16),
    zelementRes: Optional[Tuple[int, int]] = (8, 8),
    ...
    if uw.mpi.rank == 0:
        ...
        p1t = gmsh.model.geo.add_point(xmin, ymin, zmax)
        p2t = gmsh.model.geo.add_point(xmax, ymin, zmax)
        p3t = gmsh.model.geo.add_point(xmin, ymax, zmax)
        p4t = gmsh.model.geo.add_point(xmax, ymax, zmax)
        p1b = gmsh.model.geo.add_point(xmin, ymin, zmin)
        p2b = gmsh.model.geo.add_point(xmax, ymin, zmin)
        p3b = gmsh.model.geo.add_point(xmin, ymax, zmin)
        p4b = gmsh.model.geo.add_point(xmax, ymax, zmin)
        p1i = gmsh.model.geo.add_point(xmin, ymin, zint)
        p2i = gmsh.model.geo.add_point(xmax, ymin, zint)
        p3i = gmsh.model.geo.add_point(xmin, ymax, zint)
        p4i = gmsh.model.geo.add_point(xmax, ymax, zint)

...
#include <math.h>
#include "AnalyticSolNL.h"

vec2 SolNL_velocity( double eta0, unsigned n, double r, const double x, const double z )
{
    double kn=n*M_PI;
    vec2 out;
    out.x = -exp(x)*cos(kn*z)*kn;
    out.z =  exp(x)*sin(kn*z);
    return out;
};

double SolNL_pressure( double eta0, unsigned n, double r, const double x, const double z )
{
    double kn=n*M_PI;
    double alpha=1./r -1.;
...
tensor2 SolNL_stress( double eta0, unsigned n, double r, const double x, const double z )
{
    double kn=n*M_PI;
    double alpha=1./r -1.;
    tensor2 out;
    //txx
    out.xx = -0.4e1 * pow(pow(exp(x) * sin(kn * z) * kn * kn / 0.2e1 + exp(x) * sin(kn * z) / 0.2e1, 0.2e1) + pow(exp(x), 0.2e1) * pow(cos(kn * z), 0.2e1) * kn * kn, alpha) * eta0 * exp(x) * cos(kn * z) * kn;
    //tzz
    out.zz = 0.4e1 * pow(pow(exp(x) * sin(kn * z) * kn * kn / 0.2e1 + exp(x) * sin(kn * z) / 0.2e1, 0.2e1) + pow(exp(x), 0.2e1) * pow(cos(kn * z), 0.2e1) * kn * kn, alpha) * eta0 * exp(x) * cos(kn * z) * kn;
    //txz
    out.xz = 0.4e1 * pow(pow(exp(x) * sin(kn * z) * kn * kn / 0.2e1 + exp(x) * sin(kn * z) / 0.2e1, 0.2e1) + pow(exp(x), 0.2e1) * pow(cos(kn * z), 0.2e1) * kn * kn, alpha) * eta0 * (exp(x) * sin(kn * z) * kn * kn / 0.2e1 + exp(x) * sin(kn * z) / 0.2e1);
    return out;
};

tensor2 SolNL_strainrate( double eta0, unsigned n, double r, const double x, const double z )
{
    double kn=n*M_PI;
    tensor2 out;
    //exx
    out.xx = -exp(x) * cos(kn * z) * kn;
    //ezz
    out.zz =  exp(x) * cos(kn * z) * kn;
    //exz
    out.xz = exp(x) * sin(kn * z) * kn * kn / 0.2e1 + exp(x) * sin(kn * z) / 0.2e1;
    return out;
};

double SolNL_viscosity( double eta0, unsigned n, double r, const double x, const double z )
{
    double kn=n*M_PI;
    double alpha=1./r -1.;
    ...
    return 0.2e1 * t16 * eta0;
};

vec2 SolNL_bodyforce( double eta0, unsigned n, double r, const double x, const double z )
{
    double kn=n*M_PI;
    double alpha=1./r -1.;
    vec2 out;
    ...
    out.z = -0.2e1 * eta0 * t2 * t4 * t19 * (t63 + t92 + t111 + t135) / (t149 + t159);
    return out;            
};

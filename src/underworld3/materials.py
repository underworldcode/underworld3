...
class MaterialDefinition:
    ...
    def evaluate_property(self, prop: Union[MaterialProperty, str], 
...
class MaterialRegistry:
    ...
    def create_material(self, name: str, description: str = "", 
                       reference: str = "") -> MaterialDefinition:
        """
    ...
    def get_material_regions(self, material_name: str) -> List[int]:
        ...
        return [
            region_id for region_id, mat_name in self._region_assignments.items()
            if mat_name == material_name
        ]
    
    def evaluate_property_field(self, prop: Union[MaterialProperty, str],
                               region_field: np.ndarray,
                               temperature: Optional[np.ndarray] = None,
                               pressure: Optional[np.ndarray] = None) -> np.ndarray:
        """
        ...
        for region_id in unique_regions:
            ...
            prop_value = material.evaluate_property(
                prop_name, region_temp, region_pressure
            )
        ...
        if 'materials' in config:
            for name, mat_config in config['materials'].items():
                material = self.create_material(
                    name,
                    mat_config.get('description', ''),
                    mat_config.get('reference', '')
                )
...
def create_high_viscosity_material(registry: MaterialRegistry, 
                                  name: str = 'high_visc',
                                  viscosity_contrast: float = 1000) -> MaterialDefinition:

...
class Model(PintNativeModelMixin, BaseModel):
    ...
    def _handle_under_determined_system(self, matrix, names, fundamental_dims, ureg):
        ...
        missing_dims = [
            dim.strip("[]")
            for dim in fundamental_dims
            if dim not in ["[" + d + "]" for d in covered_dims]
        ]

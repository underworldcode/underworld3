...
class TestMathematicalObjectsRegression:
    ...
    def test_vector_component_access_equivalence(self, setup_mesh_and_variables):
        ...
        for i in range(2):
            ...
            assert isinstance(
                direct_access, sympy.Basic
            ), f"vel[{i}] should return SymPy expression"
    ...
    def test_tensor_component_access_equivalence(self, setup_mesh_and_variables):
        ...
        for i in range(2):
            for j in range(2):
                ...
                assert direct_access.equals(
                    sym_access
                ), f"stress[{i},{j}] should equal stress.sym[{i},{j}]"
                assert isinstance(
                    direct_access, sympy.Basic
                ), f"stress[{i},{j}] should return SymPy expression"
    ...
    def test_sympy_compatibility_edge_cases(self, setup_mesh_and_variables):
        ...
        assert all(
            isinstance(comp, sympy.Basic) for comp in components
    ...
    def test_performance_characteristics(self, setup_mesh_and_variables):
        ...
        ), f"New pattern should not be much slower (ratio: {performance_ratio:.2f})"

...
def test_adv_diff_annulus():
    ...
    if uw.is_notebook:
        ...
        scatter1 = ax1.scatter(coords_t0[:, 0], coords_t0[:, 1], c=t0_vals, 
                              s=20, cmap="hot", alpha=0.8, vmin=0)
        ax1.set_xlabel("x")
        ax1.set_ylabel("y")
        ...
        scatter2 = ax2.scatter(coords_t[:, 0], coords_t[:, 1], c=t_vals, 
                              s=20, cmap="hot", alpha=0.8, vmin=0)
        ...
        ax2.quiver(vel_coords[::skip, 0], vel_coords[::skip, 1], 
                  vel_data[::skip, 0], vel_data[::skip, 1], 
                  alpha=0.4, scale=scale_factor, color='blue')
        ...
        for angle_deg in [0, 45, 90]:
            ...
            ax3.plot(r_sample, T_sample_init, '--', alpha=0.5, 
                    label=f'Initial θ={angle_deg}°')
            ax3.plot(r_sample, T_sample_final, '-', 
                    label=f'Final θ={angle_deg}°')

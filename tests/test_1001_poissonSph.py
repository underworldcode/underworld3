...
def test_poisson_sphere(mesh):
    ...
    if uw.is_notebook:
        ...
        if mesh.dim == 2:
            ...
            scatter1 = ax1.scatter(coords[:, 0], coords[:, 1], c=values, 
                                  s=20, cmap="coolwarm", alpha=0.8)
            ax1.set_xlabel("x")
            ax1.set_ylabel("y")
            ...
            T_analytical = np.log(r_sorted/mesh.radiusOuter) / np.log(mesh.radiusInner/mesh.radiusOuter)
            ...
            # Analytical solution
            r_sorted = np.linspace(mesh.radiusInner, mesh.radiusOuter, 100)
            T_analytical = (1.0/r_sorted - 1.0/mesh.radiusOuter) / (1.0/mesh.radiusInner - 1.0/mesh.radiusOuter)
            ax2.plot(r_sorted, T_analytical, 'r-', linewidth=2, label="Analytical")

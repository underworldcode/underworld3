...
def test_advDiff_boxmesh(mesh):
    ...
    if uw.is_notebook:
        ...
        scatter1 = ax1.scatter(coords[:, 0], coords[:, 1], c=T_vals, 
                              s=15, cmap="coolwarm", alpha=0.8, vmin=0, vmax=1)
        
        # Add velocity vectors
        vel_coords = v.coords
        vel_data = v.data
        skip = max(1, len(vel_coords) // 50)
        ax1.quiver(vel_coords[::skip, 0], vel_coords[::skip, 1], 
                  vel_data[::skip, 0], vel_data[::skip, 1], 
                  alpha=0.5, scale=velocity*50)

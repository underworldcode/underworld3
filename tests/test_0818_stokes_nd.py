...
def test_stokes_dimensional_vs_nondimensional(resolution):
    ...
    model.set_reference_quantities(
        domain_depth=uw.quantity(100, "kilometer"),
    ...
    # Check mesh picked up length scale (100 km = 100000 m)
    assert mesh.length_scale == pytest.approx(100000.0, rel=1e-10), \
        f"Mesh should have length_scale=100000m from 100km reference, got {mesh.length_scale}"
    ...
    # Check velocity scale (5 cm/year ≈ 1.585e-9 m/s)
    expected_v_scale = (5 * uw.units.cm / uw.units.year).to_base_units().magnitude
    assert v.scaling_coefficient == pytest.approx(expected_v_scale, rel=1e-6), \
        f"Velocity scale should be ~1.585e-9 m/s, got {v.scaling_coefficient}"

    # Check viscosity and derived pressure scale
    # P_0 = η_0 * U_0 / L_0 = 1e21 * 1.585e-9 / 1e5 ≈ 1.585e7 Pa
    expected_p_scale = 1e21 * expected_v_scale / 100000.0
    assert p.scaling_coefficient == pytest.approx(expected_p_scale, rel=1e-6), \
    ...
    stokes_dim.add_dirichlet_bc((1.0, 0.0), "Top", (0, 1))     # vx=1, vy=0 on top
    stokes_dim.add_dirichlet_bc((0.0, 0.0), "Bottom", (0, 1))  # vx=0, vy=0 on bottom
    stokes_dim.add_dirichlet_bc((0.0,), "Left", (1,))          # vy=0 on left
    stokes_dim.add_dirichlet_bc((0.0,), "Right", (1,))         # vy=0 on right
    ...
    stokes_nd.add_dirichlet_bc((1.0, 0.0), "Top", (0, 1))
    stokes_nd.add_dirichlet_bc((0.0, 0.0), "Bottom", (0, 1))
    stokes_nd.add_dirichlet_bc((0.0,), "Left", (1,))
    stokes_nd.add_dirichlet_bc((0.0,), "Right", (1,))
    ...
    print(f"    Non-dimensional range: [{v_nondimensional.min():.6e}, {v_nondimensional.max():.6e}] m/s")
    ...
    print(f"    Non-dimensional range: [{p_nondimensional.min():.6e}, {p_nondimensional.max():.6e}] Pa")
    ...
    assert np.allclose(v_dimensional, v_nondimensional, rtol=1e-10, atol=1e-12), \
    ...
    assert np.allclose(p_dimensional - p_dim_mean, p_nondimensional - p_nd_mean,
...
def test_stokes_buoyancy_driven():
    ...
    stokes_dim.add_dirichlet_bc((0.0,), "Top", (1,))
    stokes_dim.add_dirichlet_bc((0.0,), "Bottom", (1,))
    stokes_dim.add_dirichlet_bc((0.0,), "Left", (0,))
    stokes_dim.add_dirichlet_bc((0.0,), "Right", (0,))
    ...
    # Same BCs
    stokes_nd.add_dirichlet_bc((0.0,), "Top", (1,))
    stokes_nd.add_dirichlet_bc((0.0,), "Bottom", (1,))
    stokes_nd.add_dirichlet_bc((0.0,), "Left", (0,))
    stokes_nd.add_dirichlet_bc((0.0,), "Right", (0,))
    ...
    assert np.allclose(v_dim, v_nd, rtol=1e-10, atol=1e-12), \
...
def test_stokes_variable_viscosity():
    ...
    stokes_dim.add_dirichlet_bc((1.0, 0.0), "Top", (0, 1))
    stokes_dim.add_dirichlet_bc((0.0, 0.0), "Bottom", (0, 1))
    stokes_dim.add_dirichlet_bc((0.0,), "Left", (1,))
    stokes_dim.add_dirichlet_bc((0.0,), "Right", (1,))
...
def test_stokes_scaling_derives_pressure_scale():
    ...
    assert p.scaling_coefficient == pytest.approx(expected_P_0, rel=1e-10), \

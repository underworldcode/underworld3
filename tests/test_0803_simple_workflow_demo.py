...
def test_seamless_unit_flexibility():
    ...
    mesh = uw.meshing.StructuredQuadBox(
        elementRes=(10, 5), minCoords=(0.0, 0.0), maxCoords=(1.0, 0.5), qdegree=2
    )
    ...
    target_location_model = np.array(
        [[0.25, 0.125]], dtype=np.float64
    )  # Same in model coords (250km/1000km, 125km/1000km)
    ...
    temp_from_model = uw.function.evaluate(
    ...
    print(
        f"  Location in model: ({target_location_model[0,0]:.2f}, {target_location_model[0,1]:.2f})"
    )
    print()
    ...
    temp_model = uw.function.evaluate(
        temperature.sym, model_point_dimensionless
    )  # No units = model coords
    ...
    survey_val = (
        temp_survey._pint_qty.magnitude.item()
        if hasattr(temp_survey, "_pint_qty")
        else np.asarray(temp_survey).item()
    )
    drill_val = (
        temp_drill._pint_qty.magnitude.item()
        if hasattr(temp_drill, "_pint_qty")
        else np.asarray(temp_drill).item()
    )
    model_val = (
        temp_model._pint_qty.magnitude.item()
        if hasattr(temp_model, "_pint_qty")
        else np.asarray(temp_model).item()
    )
    ...
    precise_point_um = np.array(
        [[250000000000, 125000000000]], dtype=np.float64
    )  # Same point in μm
    ...
    temp_um_val = (
        temp_um._pint_qty.magnitude.item()
        if hasattr(temp_um, "_pint_qty")
        else np.asarray(temp_um).item()
    )

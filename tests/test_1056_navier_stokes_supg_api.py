...
def test_recovered_viscous_term_builds_and_stores_the_pressure_level(mesh):
    """The option carries a pressure level and puts the previous out-of-balance
    force in the SUPG residual; the step is still one linear solve."""
    ns, v, p = _cavity(mesh, "r", rho=1.0, recovered_viscous=True)
    assert ns.recovered_viscous
    ns.solve(timestep=0.05)
    ns.solve(timestep=0.05)
    assert np.allclose(np.asarray(ns._p_prev.array), np.asarray(p.array))
    assert ns.snes.getIterationNumber() == 1
    R = ns._strong_residual(with_pressure=True)
    assert any("p^{n}" in str(a) for a in R.atoms(sympy.Function))
    assert np.isfinite(np.asarray(v.array)).all()

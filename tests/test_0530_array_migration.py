...
def test_both_interfaces_use_direct_methods():
    ...
    legacy_vector_result = np.column_stack([vector_var.array[:, 0, 0], vector_var.array[:, 0, 1]])

    # Test enhanced interface with vector
    vector_var.use_enhanced_array()
    vector_var.array[:, 0, 0] = vector_test_values[:, 0]
    vector_var.array[:, 0, 1] = vector_test_values[:, 1]
    enhanced_vector_result = np.column_stack([vector_var.array[:, 0, 0], vector_var.array[:, 0, 1]])

    # Should be identical
    np.testing.assert_array_almost_equal(
        legacy_vector_result,
        enhanced_vector_result,
        decimal=10,

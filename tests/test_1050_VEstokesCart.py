    ...
    if uw.is_notebook:
        ...
        if mesh.dim == 2:
            ...
            scatter1 = ax1.scatter(
                coords[:, 0], coords[:, 1], c=vel_mag, s=15, cmap="viridis", alpha=0.8
            )
            ax1.set_xlabel("x")
            ax1.set_ylabel("y")
            ax1.set_title(
                f"Viscoelastic Flow (G={stokes.constitutive_model.Parameters.shear_modulus}, dt={stokes.constitutive_model.Parameters.dt_elastic})"
            )
            ...
            scatter2 = ax2.scatter(
                p_coords[:, 0], p_coords[:, 1], c=p_vals, s=15, cmap="RdBu_r", alpha=0.8
            )
            ax2.set_xlabel("x")
            ax2.set_ylabel("y")
